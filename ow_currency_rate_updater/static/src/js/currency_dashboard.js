...
export class OwCurrencyDashboard extends Component {
    ...
    currentCompanyId() {
    	if (this.state.company) return this.state.company.id;
    	if (session.user_companies && session.user_companies.current_company) {
            return session.user_companies.current_company;
        }
        if (session.user_context && session.user_context.allowed_company_ids) {
            return session.user_context.allowed_company_ids[0];
        }
