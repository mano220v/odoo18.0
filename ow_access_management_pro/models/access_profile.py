...
class AccessModelLine(models.Model):
    ...
    _sql_constraints = [("unique_profile_model", "UNIQUE(profile_id, model_id)", "A model can appear only once per profile.")]
...
class AccessFieldLine(models.Model):
    ...
    _sql_constraints = [("unique_profile_field", "UNIQUE(profile_id, field_id)", "A field can appear only once per profile.")]
